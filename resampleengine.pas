...
begin
  if Length(Value) = 0 then Exit; Stream.Write(ID[1], 4); Len := Length(Value) + 1; Stream.Write(Len, 4);
  Stream.Write(Value[1], Length(Value)); Stream.Write(Zero, 1); if (Len mod 2 <> 0) then Stream.Write(Zero, 1);
end;
...
begin
  ...
  try
    A1 := LoadWav(f1, SR1); if f2 <> '' then A2 := LoadWav(f2, SR2) else begin SetLength(A2, Length(A1)); for c := 0 to High(A2) do begin SetLength(A2[c], 1); A2[c][0] := 1.0; end; SR2 := SR1; end;
