...
var
  _pffft_new_setup: function(N: Integer; t: Integer): Pointer; cdecl = nil;
  _pffft_destroy_setup: procedure(s: Pointer); cdecl = nil;
  _pffft_transform_ordered: procedure(s: Pointer; input, output, work: PSingle; d: Integer); cdecl = nil;
  _pffft_zconvolve_accumulate: procedure(s: Pointer; a, b, ab: PSingle; scaling: Single); cdecl = nil;
  _pffft_aligned_malloc: function(b: NativeUInt): Pointer; cdecl = nil;
  _pffft_aligned_free: procedure(p: Pointer); cdecl = nil;
...
begin
  ...
  if PHandle <> NilHandle then begin
    _pffft_new_setup := GetProcAddress(PHandle, 'pffft_new_setup');
    _pffft_destroy_setup := GetProcAddress(PHandle, 'pffft_destroy_setup');
    _pffft_transform_ordered := GetProcAddress(PHandle, 'pffft_transform_ordered');
    _pffft_zconvolve_accumulate := GetProcAddress(PHandle, 'pffft_zconvolve_accumulate');
    _pffft_aligned_malloc := GetProcAddress(PHandle, 'pffft_aligned_malloc');
    _pffft_aligned_free := GetProcAddress(PHandle, 'pffft_aligned_free');
  end;
end;
...
begin
  OutChans := IfThen(ForceMono, 1, Length(Data)); FillChar(H, SizeOf(H), 0);
  ...
  SetLength(FErrorMem, OutChans); FS := TFileStream.Create(FileName, fmCreate);
  try
    FS.Write(H, SizeOf(H));
    for i := 0 to High(Data) do for c := 0 to OutChans - 1 do
      if Bits = 16 then begin s16 := Round(Data[c][i] * 32767); FS.Write(s16, 2); end
...
var StartTime: Int64; f1, f2, fOut, Msg: string; A1, A2, Res: TAudioData; SR1, SR2, bOut, c, TargetSR, TruncLen: Integer; TMethod: TMethod;
begin
  ...
  try
    ...
    TMethod.Code := @TTaraDSPApp.SaveWav; TMethod.Data := Self;
