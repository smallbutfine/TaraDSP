/*
  ...
  clang -o test_pffft -I /opt/intel/mkl/include -DHAVE_FFTW -DHAVE_VECLIB -DHAVE_MKL  -O3 -Wall -W pffft.c test_pffft.c fftpack.c -L/opt/homebrew/lib -I/opt/homebrew/include/ -lfftw3f -framework Accelerate /opt/intel/mkl/lib/libmkl_{intel_lp64,sequential,core}.a
...
float norm_inf_rel(float *v, float *w, int N) {
  ...
  int k;
  for (k=0; k < N; ++k) {
    max_w = MAX_OF(max_w, fabsf(w[k]));
    max_diff = MAX_OF(max_diff, fabsf(w[k] - v[k]));
  }
  assert(max_w > 0);
...
void pffft_validate_N(int N, int cplx) {
  ...
  size_t Nbytes = (size_t)Nfloat * sizeof(float);
  ...
  for (pass=0; pass < 2; ++pass) {
    ...
    for (k = 0; k < Nfloat; ++k) ref_max = MAX_OF(ref_max, fabsf(ref[k]));
    ...
    // quick test of the circular convolution in fft domain
    {
      ...
      for (k=0; k < Nfloat; ++k) {
        float d = fabsf(tmp[k] - tmp2[k]), e = fabsf(tmp[k]);
...
void pffft_validate(int cplx) {
  static int Ntest[] = { 16, 32, 64, 96, 128, 160, 192, 256, 288, 384, 5*96, 512, 576, 5*128, 800, 864, 1024, 2048, 2592, 4000, 4096, 12000, 1024*3*5, 36864, 0 };
...
void show_output(const char *name, int N, int cplx, double flops, double t0, double t1, int max_iter) {
  double mflops = flops/1e6/(t1 - t0 + 1e-16);
...
void benchmark_ffts(int N, int cplx) {
...
#ifdef HAVE_VECLIB
  size_t log2N = (size_t)(log(N)/log(2) + 0.5f);
  if (N == (1<<log2N)) {
    FFTSetup setup;
...
#ifdef HAVE_FFTW
  {
    ...
    fftw_complex *in = (fftw_complex*) fftwf_malloc(sizeof(fftw_complex) * (size_t)N);
    fftw_complex *out = (fftw_complex*) fftwf_malloc(sizeof(fftw_complex) * (size_t)N);
    memset(in, 0, sizeof(fftw_complex) * (size_t)N);
    unsigned flags = (N < 40000 ? FFTW_MEASURE : FFTW_ESTIMATE);  // measure takes a lot of time on largest ffts
...
void validate_pffft_simd(void); // a small function inside pffft.c that will detect compiler bugs with respect to simd instruction
#endif

int main(int argc, char **argv) {
  int Nvalues[] = { 64, 96, 128, 160, 192, 256, 384, 5*96, 512, 5*128, 3*256, 800, 1024, 2048, 2400, 4096, 8192, 9*1024, 1024*3*5, 16384, 32768, 256*1024, 1024*1024, 2<<24, 2<<25, -1 };
  int Nmax = 1014*1024;
  int i;
