🌐 IRConvolverPro Documentation (v1.2)
Welcome to the official documentation for IRConvolverPro, the professional high-performance FFT toolkit for Impulse Response (IR) processing, developed in Free Pascal.
1. Introduction
IRConvolverPro is designed for audio engineers and hardware modeler users (e.g., Valeton GP-200, Line6 Helix) who require surgical precision, high-speed batch processing, and mastering-grade output quality.
Key Technologies
...
Run IRConvolverPro-CLI -h to see all options.
Basic Syntax
bash

IRConvolverPro-CLI -i1 <source> [-i2 <ir_file>] -o <output> [options]
...
When exporting to 16-bit, IRConvolverPro applies uncorrelated TPDF dither per channel. This preserves stereo width and depth. The integrated 2nd-Order Noise Shaper pushes quantization noise into the ultra-high frequency range (>16kHz), ensuring an "inky black" background.
...
7. License
IRConvolverPro is released under the BSD 3-Clause License. You are free to use, modify, and distribute the software as long as the original copyright notice is preserved.